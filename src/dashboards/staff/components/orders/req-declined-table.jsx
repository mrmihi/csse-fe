...
const RequisitionDeclinedOrder = () => {
  const [declinedRequests, setDeclinedRequests] = useState([]);

  const fetchData = async () => {
    try {
      const response = await SupplierService.getAllDeclinedOrders();
      if (response.status === 200) {
        setDeclinedRequests(response.data.results);
      } else {
  ...
  return (
    <>
      <PageTitle title={"Declined Requisitions"} />
      <OrderStatsSection />
      <MaterialReactTable
        columns={columns}
...
export default RequisitionDeclinedOrder;
